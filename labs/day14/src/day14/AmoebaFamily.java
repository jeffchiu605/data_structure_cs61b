...
public class AmoebaFamily {
	...
	public static void replaceNameHelper(String currentName, String newName,
		...
		if (currentAmoeba.myName.equals(currentName)) {
			...
			Iterator<Amoeba> iter = currentAmoeba.myChildren.iterator();
			while (iter.hasNext()) {
				AmoebaFamily.replaceNameHelper(currentName, newName, iter.next());
			}
		}
	}
	...
	public static void printFlatHelper(Amoeba currentAmoeba) {
		// Your goal is to make this as similar as possible to addChildHelper
		System.out.println(currentAmoeba);
		Iterator<Amoeba> iter = currentAmoeba.myChildren.iterator();
			while (iter.hasNext()) {
	...
	public static void printHelper(Amoeba currentAmoeba, int n) { 
		String printer = "";
		for (int i = 0; i < n; i++) {
				printer = printer + " ";
		}
		printer = printer + currentAmoeba.myName;
		System.out.print(printer);
		System.out.println("");
		for (Amoeba child: currentAmoeba.myChildren)
		printHelper(child, n+4);
	}
